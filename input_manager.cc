...
void InputManager::m_eventcursor()
{
	...
	for (auto it = m_Listener.begin(); it != m_Listener.end(); ++it)
	{
		if (!(*it))
			continue;
		else if ((*it)->MouseMove(MouseEvent(m_curx, m_cury)))
			return;
	}
}
...
void InputManager::m_eventmouse(const eMouseKeyCodes Code, bool press)
{
	for (auto it = m_Listener.begin(); it != m_Listener.end(); ++it)
	{
		...
		if (press == true)
		{
			if ((*it)->MousePressed(MouseEventClick(Code, m_curx, m_cury)))
				return;
		}
		else
		{
			if ((*it)->MouseReleased(MouseEventClick(Code, m_curx, m_cury)))
				return;
		}
	}
}
...
void InputManager::m_mousewheel(short Value)
{
	...
	for (auto it = m_Listener.begin(); it != m_Listener.end(); ++it)
	{
		if (!(*it))
			continue;
		else if ((*it)->MouseWheel(MouseEventWheel(m_MouseWheel, m_curx, m_cury)))
			return;
	}
}
...
void InputManager::m_eventkey(const eKeyCodes KeyCode, const wchar_t ch, bool press)
{
	for (auto it = m_Listener.begin(); it != m_Listener.end(); ++it)
	{
		...
		if (press)
		{
			if ((*it)->KeyPressed(KeyEvent(ch, KeyCode)))
				return;
		}
		else
		{
			if ((*it)->KeyReleased(KeyEvent(ch, KeyCode)))
				return;
		}
	}
}
